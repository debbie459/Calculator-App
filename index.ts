const inputEL  = document.querySelector("#input-el") as HTMLInputElement | null
...
const operatorsArray = [buttonPlus?.textContent, buttonMinus?.textContent, buttonDivide?.textContent, buttonMultiply?.textContent]
...
let finalOperation = (number1: number, number2 : number, operands:string) =>{
    if (operands === '+'){
...
deleteBtn?.addEventListener("click", ()=>{
        let currentInputItems = inputEL?.value
        console.log("hey, this button works!")
        let currentItemsArray = currentInputItems?.split("")
        console.log(currentItemsArray)
        for (let i=0; i<1; i++){
            currentInputItems = currentItemsArray?.pop()
            inputEL!.value = ""
            console.log(currentItemsArray)
        }
